...
internal class LuaLspClient(string serverPath) : IDisposable {
    ...
    private void Listen() {
        ...
        while (IsAlive) {
            ...
            try {
                ...
                else if (obj["method"] != null) NotificationReceived?.Invoke(obj["method"]!.Value<string>()!, obj["params"] ?? new JObject());
            } catch {
                /**/
            }
        }
    ...
    private void ListenError() {
        ...
        try {
            while (IsAlive) _process?.StandardError.ReadLine();
        } catch {
            /**/
        }
    }
