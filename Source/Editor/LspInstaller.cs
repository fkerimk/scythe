...
internal static class LspInstaller {
    ...
    public static async void Start() {
        ...
        try {
            ...
            try { await InstallAsync(); } catch (Exception e) {
    ...
    private static async Task InstallAsync() {
        ...
        if (archivePath.EndsWith(".zip")) await ZipFile.ExtractToDirectoryAsync(archivePath, lspDir, true); else {

            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                 ExtractTarGz(archivePath);
            else await System.Diagnostics.Process.Start("tar", $"-xzf \"{archivePath}\" -C \"{lspDir}\"").WaitForExitAsync();
        }
