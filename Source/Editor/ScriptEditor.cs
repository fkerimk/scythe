...
internal unsafe class ScriptEditor : Viewport {
    ...
    private static float _shakeTime, _shakePower, _wedgeAnim, _repeatTimer, _lspUpdateTimer, _didChangeTimer, _restartTimer, _timer, _acCooldown;
    ...
    private static int _lastWedgeLine = -1, _lastWedgeChar = -1, _acSelectedIndex, _acScrollIndex, _acLine, _acStartChar, _sigLine, _sigChar, _completionRequestId = -1, _sigRequestId = -1, _hoverRequestId = -1, _semanticTokensRequestId = -1;

    private static Vector2 _shakeOffset = Vector2.Zero, _lastMousePos = Vector2.Zero;
    ...
    public ScriptEditor() : base("Script Editor") {
        ...
        CustomStyle = new CustomStyle { WindowPadding = Vector2.Zero, CellPadding = Vector2.Zero, SeparatorTextPadding = Vector2.Zero };
    ...
    private void CloseTab(int i) {
        ...
        if (_lsp is { IsAlive: true }) _lsp.SendNotification("textDocument/didClose", new { textDocument = new { uri = _tabs[i].Uri } });
    ...
    private void UpdateAutocomplete(JToken r) {
        ...
        foreach (var i in r["items"] ?? r) {
            ...
            _acItems.Add(new CompletionItem { Label = (string)i["label"]!, InsertText = (string)(i["insertText"] ?? i["label"])!, InsertTextFormat = (int)(i["insertTextFormat"] ?? 1), Kind = (int)(i["kind"] ?? 0) });
        }

        var pref = GetCurrentWordPrefix();

        if (!string.IsNullOrEmpty(pref)) _acItems = _acItems.Where(i => i.Label.StartsWith(pref, StringComparison.OrdinalIgnoreCase)).ToList();
    ...
    private void OnType(bool major = false, bool backspace = false, bool isEnter = false, bool fromAc = false) {
        ...
        if (should && (pref.Length >= 1 || isT)) {
            ...
            _completionRequestId = _lsp.SendRequest("textDocument/completion", new { textDocument = new { uri = ActiveTab.Uri }, position = new { line = ActiveTab.CursorLine, character = ActiveTab.CursorChar }, context = new { triggerKind = isT ? 2 : 1, triggerCharacter = isT ? cB.ToString() : null } });
        ...
        if (cB == '(' || cB == ',') {
            ...
            _sigRequestId = _lsp.SendRequest("textDocument/signatureHelp", new { textDocument = new { uri = ActiveTab.Uri }, position = new { line = ActiveTab.CursorLine, character = ActiveTab.CursorChar } });
    ...
    private void SyncChanges() {
        ...
        _lsp.SendNotification("textDocument/didChange", new { textDocument = new { uri = ActiveTab.Uri, version = ActiveTab.LspVersion++ }, contentChanges = new[] { new { text = string.Join("\n", ActiveTab.Lines) } } });
    }
    ...
    private void HandleInput() {
        ...
        KeyboardKey[] keys = [KeyboardKey.Backspace, KeyboardKey.Delete, KeyboardKey.Enter, KeyboardKey.KpEnter, KeyboardKey.Left, KeyboardKey.Right, KeyboardKey.Up, KeyboardKey.Down, KeyboardKey.Escape, KeyboardKey.Tab, KeyboardKey.Home, KeyboardKey.End];
        ...
        for (var n = 0; n < io.InputQueueCharacters.Size; n++) {
            ...
            if (ActiveTab.CursorChar < ActiveTab.Lines[ActiveTab.CursorLine].Length && ActiveTab.Lines[ActiveTab.CursorLine][ActiveTab.CursorChar] == c && c is ')' or '}' or ']' or '"' or '\'') {
    ...
    private void PerformAction(KeyboardKey k, bool s) {
        ...
        if (k == KeyboardKey.Left || k == KeyboardKey.Right || k == KeyboardKey.Up || k == KeyboardKey.Down) _showAutoComplete = false;

        if (!s && k != KeyboardKey.Backspace && k != KeyboardKey.Delete && k != KeyboardKey.Enter && k != KeyboardKey.KpEnter && k != KeyboardKey.Tab && HasSelection()) {
        ...
        if (!s && k != KeyboardKey.Backspace && k != KeyboardKey.Delete && k != KeyboardKey.Enter && k != KeyboardKey.Tab && HasSelection()) ClearSelection();
    }
    ...
    private void ClearSelection() { ActiveTab.SelectionStartLine = ActiveTab.SelectionStartChar = -1; }

    private (int sL, int sC, int eL, int eC) GetSelectionRange() => !HasSelection()
        ? (0, 0, 0, 0)
        : (ActiveTab.SelectionStartLine < ActiveTab.CursorLine || (ActiveTab.SelectionStartLine == ActiveTab.CursorLine && ActiveTab.SelectionStartChar <= ActiveTab.CursorChar))
    ...
    private void HandleEnter() {
        ...
        if (left.TrimEnd().EndsWith("do") || left.TrimEnd().EndsWith("then") || left.TrimEnd().EndsWith("function()") || left.TrimEnd().EndsWith("{")) indent += "    ";
    ...
    private void HandleBackspace() {
        ...
        if (ActiveTab.CursorChar > 0) {
            ...
            if (ActiveTab.CursorChar < l.Length) {
                ...
                if ((cL == '(' && cR == ')') || (cL == '{' && cR == '}') || (cL == '[' && cR == ']') || (cL == '"' && cR == '"') || (cL == '\'' && cR == '\'')) {
    ...
    private void ClampCursor() {
        ...
        if (ActiveTab.SelectionStartLine != -1) {
            ...
            ActiveTab.SelectionStartChar = Math.Clamp(ActiveTab.SelectionStartChar, 0, ActiveTab.Lines[ActiveTab.SelectionStartLine].Length);
        }
    }
    ...
    private void DrawEditorContent(Vector2 m, float fS, float lS, Vector2 sO) {
        ...
        foreach (var p in Particles) {
            ...
            if (p.Size * ActiveTab.Zoom > 2) DrawCircleV(sP, p.Size * 1.5f * ActiveTab.Zoom, Fade(p.Color, 0.1f * (p.Life / 0.6f)));
        }
    ...
    private void DrawDragSelectionGhost(Vector2 m, float fS, float lS, Vector2 bP, Vector2 sO) {
        ...
        if (GetCursorFromMouse(m, fS, lS, sO, out var tL, out var tC)) {
            var (sL, sC, eL, eC) = GetSelectionRange();
            if (!((tL > sL && tL < eL) || (tL == sL && tC > sC && (tL != eL)) || (tL == eL && tC < eC && (tL != sL)) || (sL == eL && tL == sL && tC >= sC && tC <= eC))) DrawDropMarker(bP + new Vector2(tL < ActiveTab.Lines.Count ? MeasureTextEx(EditorFont, ActiveTab.Lines[tL][..Math.Min(tC, ActiveTab.Lines[tL].Length)], fS, 1).X : 0, tL * lS), fS, Color.Yellow);
        }
    }
    ...
    private Vector2 GetCursorWorldPosition(Vector2 m, float f, float l) => m + new Vector2(MeasureTextEx(EditorFont, ActiveTab.Lines[Math.Clamp(ActiveTab.CursorLine, 0, ActiveTab.Lines.Count - 1)][..Math.Clamp(ActiveTab.CursorChar, 0, ActiveTab.Lines[Math.Clamp(ActiveTab.CursorLine, 0, ActiveTab.Lines.Count - 1)].Length)], f, 1).X, Math.Clamp(ActiveTab.CursorLine, 0, ActiveTab.Lines.Count - 1) * l);
    ...
    private void DrawAutocompletePopup(Vector2 m, float fS, float lS, Vector2 sO) {
        ...
        for (var i = _acScrollIndex; i < Math.Min(_acScrollIndex + mV, _acItems.Count); i++) {
            ...
            if (MeasureTextEx(EditorFont, l, 18 * ActiveTab.Zoom, 1).X > mTw) {
                while (l.Length > 3 && MeasureTextEx(EditorFont, l + "...", 18 * ActiveTab.Zoom, 1).X > mTw) l = l[..^1];
                l += "...";
            }

            DrawTextEx(EditorFont, l, new Vector2(pP.X + 30 * ActiveTab.Zoom, iY + (iH - 18 * ActiveTab.Zoom) / 2), 18 * ActiveTab.Zoom, 1, sel ? Color.White : iC);
        }
    }
    ...
    private void DrawTooltip(Vector2 mO) {
        ...
        for (var i = 0; i < lines.Length; i++) {
            ...
            if (code) {
                ...
                var color = lines[i].Contains("error", StringComparison.OrdinalIgnoreCase) ? Color.Red : (lines[i].Contains("warning", StringComparison.OrdinalIgnoreCase) ? Color.Orange : (lines[i].StartsWith("(global)") || lines[i].StartsWith("(local)") ? new Color(130, 200, 255, 255) : Color.White));
    ...
    private bool IsInsideSelection(Vector2 wM, Vector2 m, float f, float l) {
        ...
        var eX = m.X + (line == eL ? MeasureTextEx(EditorFont, lT[..eC], f, 1).X : MeasureTextEx(EditorFont, lT, f, 1).X);
