...
internal class Preview : Viewport {
    ...
    private void DrawOverlayUi(Asset asset) {
        ...
        if (MeasureTextEx(font, displayLabel, 18, 1).X > maxLabelWidth) {
            ...
            while (displayLabel.Length > 0 && MeasureTextEx(font, "..." + displayLabel, 18, 1).X > maxLabelWidth)
                displayLabel = displayLabel.Substring(1);
        ...
        if (leftHover) {
            ...
            switch (asset) {
                ...
                case MaterialAsset:
                    _currentPrimitiveIndex = (_currentPrimitiveIndex - 1 + count) % count;
                    break;
                    
                case ModelAsset:
                    _currentAnimationIndex = (_currentAnimationIndex - 1 + count) % count; _animationTime = 0;
                    break;
            }
        }
            ...
            switch (asset) {
                ...
                case MaterialAsset:
                    _currentPrimitiveIndex = (_currentPrimitiveIndex + 1) % count;
                    break;
                    
                case ModelAsset:
                    _currentAnimationIndex = (_currentAnimationIndex + 1) % count; _animationTime = 0;
                    break;
            }
        }
    ...
    private static Mesh GetPrimitiveMesh(int index) {
        ...
        switch (index) {
            ...
            case 0: _previewSphere ??= GenMeshSphere(0.5f, 64, 64); return _previewSphere.Value;
            case 1: _previewCube ??= GenMeshCube(0.8f, 0.8f, 0.8f); return _previewCube.Value;
            case 2: _previewPlane ??= GenMeshPlane(1.0f, 1.0f, 1, 1); return _previewPlane.Value;
            case 3: _previewCylinder ??= GenMeshCylinder(0.4f, 0.8f, 32); return _previewCylinder.Value;
            case 4: _previewTorus ??= GenMeshTorus(0.2f, 0.4f, 32, 32); return _previewTorus.Value;
    ...
    public static void UpdateThumbnail(Asset? asset) {
        ...
        try {
            ...
            switch (asset) {
                ...
                case TextureAsset tex:
                    GenerateTextureThumbnail(tex);
                    break;
                
                case MaterialAsset or ModelAsset:
                    Generate3DThumbnail(asset);
                    break;
            }
            
        } finally { _isProcessingThumbnail = false; }
    }
    ...
    private static unsafe void GenerateTextureThumbnail(TextureAsset tex) {
        ...
        int newW, newH;
        if (img.Width > img.Height) { newW = 64; newH = (int)((float)img.Height / img.Width * 64); }
        else { newH = 64; newW = (int)((float)img.Width / img.Height * 64); }
