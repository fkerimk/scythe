...
internal static class ViewSettings {
    ...
    private static IEnumerable<(FieldInfo field, Viewport value)> GetViewports() {
        
        return typeof(Editor)
            .GetFields(BindingFlags.Public | BindingFlags.Static)
            .Where(f => typeof(Viewport).IsAssignableFrom(f.FieldType))
            .Select(f => (f, (Viewport)f.GetValue(null)!));
    }
    ...
    public static void Save() {
        ...
        foreach (var (field, viewport) in GetViewports())
            settings[field.Name] = viewport.IsOpen;
