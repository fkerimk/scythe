...
internal struct VertexBoneData {
    ...
    public void AddBoneData(int id, float weight) {
        ...
        if (weight <= 0) return;
        if (Weight0 <= 0) { Bone0 = id; Weight0 = weight; }
        else if (Weight1 <= 0) { Bone1 = id; Weight1 = weight; }
        else if (Weight2 <= 0) { Bone2 = id; Weight2 = weight; }
        else if (Weight3 <= 0) { Bone3 = id; Weight3 = weight; }
    }
}
...
internal static class AssimpLoader {
    ...
    public static (List<AssimpMesh> Meshes, List<BoneInfo> Bones, ModelNode Root, Matrix4x4 GlobalInverse, List<AnimationClip> Animations) Load(string path) {
        ...
        foreach (var mesh in scene.Meshes)
            meshes.Add(ProcessMesh(mesh, bones, boneMapping));
        ...
        foreach (var anim in scene.Animations)
            animations.Add(ProcessAnimation(anim));
    ...
    private static unsafe AssimpMesh ProcessMesh(Assimp.Mesh mesh, List<BoneInfo> bones, Dictionary<string, List<int>> boneMapping) {
        ...
        for (var i = 0; i < mesh.VertexCount; i++) {
            ...
            if (mesh.HasTextureCoords(0))
                am.TexCoords[i] = new Vector2(mesh.TextureCoordinateChannels[0][i].X, mesh.TextureCoordinateChannels[0][i].Y);
        }
        ...
        foreach (var bone in mesh.Bones) {
            ...
            foreach (var idx in indices) {
                ...
                if (!MatricesAreEqual(bones[idx].Offset, offset)) continue;
            ...
            foreach (var weight in bone.VertexWeights)
                am.BoneData[weight.VertexID].AddBoneData(boneIndex, weight.Weight);
        }
        ...
        // Fill initial data
        fixed (Vector3* v = am.Vertices)
            Buffer.MemoryCopy(v, am.RlMesh.Vertices, (long)am.RlMesh.VertexCount * 3 * sizeof(float), (long)am.RlMesh.VertexCount * 3 * sizeof(float));
        
        fixed (Vector3* n = am.Normals)
            Buffer.MemoryCopy(n, am.RlMesh.Normals, (long)am.RlMesh.VertexCount * 3 * sizeof(float), (long)am.RlMesh.VertexCount * 3 * sizeof(float));
        
        fixed (Vector2* t = am.TexCoords)
            Buffer.MemoryCopy(t, am.RlMesh.TexCoords, (long)am.RlMesh.VertexCount * 2 * sizeof(float), (long)am.RlMesh.VertexCount * 2 * sizeof(float));
        
        for (var i = 0; i < am.Indices.Length; i++)
            am.RlMesh.Indices[i] = (ushort)am.Indices[i];
    ...
    private static ModelNode ProcessNode(Node node) {
        ...
        foreach (var child in node.Children)
            mn.Children.Add(ProcessNode(child));
    ...
    private static AnimationClip ProcessAnimation(Assimp.Animation anim) {
        ...
        var clip = new AnimationClip {
            ...
            Name = anim.Name,
            TicksPerSecond = anim.TicksPerSecond != 0 ? anim.TicksPerSecond : 25.0
        };
    ...
    private static bool MatricesAreEqual(Matrix4x4 a, Matrix4x4 b) {
        ...
        return Math.Abs(a.M11 - b.M11) < epsilon && Math.Abs(a.M12 - b.M12) < epsilon && Math.Abs(a.M13 - b.M13) < epsilon && Math.Abs(a.M14 - b.M14) < epsilon &&
               Math.Abs(a.M21 - b.M21) < epsilon && Math.Abs(a.M22 - b.M22) < epsilon && Math.Abs(a.M23 - b.M23) < epsilon && Math.Abs(a.M24 - b.M24) < epsilon &&
               Math.Abs(a.M31 - b.M31) < epsilon && Math.Abs(a.M32 - b.M32) < epsilon && Math.Abs(a.M33 - b.M33) < epsilon && Math.Abs(a.M34 - b.M34) < epsilon &&
               Math.Abs(a.M41 - b.M41) < epsilon && Math.Abs(a.M42 - b.M42) < epsilon && Math.Abs(a.M43 - b.M43) < epsilon && Math.Abs(a.M44 - b.M44) < epsilon;
    }

    private static Matrix4x4 ToNumerics(this Assimp.Matrix4x4 m) {
